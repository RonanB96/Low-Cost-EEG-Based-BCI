...
class BCI(object):
    def __init__(self, win_size, freq_array, checker_size, checker_cycles, checker_tex, pipe):
        ...
        self.Yes_size = self.checker_size
        self.No_size = self.checker_size
        self.update_boxes = 0
        ...
        self.selection_boxes.append(visual.TextStim(self.win,
                                                    text='No',
                                                    color=[-1, -1, -1],
                                                    height=self.No_size/2,
                                                    pos=self.stim[1].pos))

        self.instructions_text1 = 'Recording Baseline, please stare at this text until its changes'
        self.instructions_text2 = 'Close your eyes to answer "Yes" and keep them open to answer "No"'
...
#        self.mouse.clickReset()
    ...
    def draw_screen(self):
        if self.update_boxes:
            self.stim[0].size = self.Yes_size
            self.stim[1].size = self.No_size
            self.selection_boxes[0].height = self.Yes_size/2
            self.selection_boxes[1].height = self.No_size/2
            self.update_boxes = 0
        for i, x in enumerate(self.stim):
            self.stim[i].draw()
            self.selection_boxes[i].draw()
    ...
    def pipeReceive(self):
        ...
        max_baseline_time = 10   # time to get baseline
        ...
        freq_sig_snr = np.zeros(freq_array_len)
        # SNR of last two fft
        ...
        alpha_index = np.argwhere(self.freq_array == 10)
                ...
                for i in range(0, len(freq_axis)):
                    ...
                    if freq_axis[i] >= freq_end:
                        if self.setting_baseline:
                            ...
                            freq_sig_snr[index] = freq_max / freq_sig_val_thresh[index]
                        break
            ...
            if self.setting_baseline:
                ...
                if self.baseline_count == max_baseline_count:
                    ...
                    for i in range(freq_array_len):
                        ...
                        fitted_cdf = st.lognorm.cdf(x, s=parameters_l[0], loc=parameters_l[1], scale=parameters_l[2])
                        ...
                        freq_sig_val_thresh[i] = mean+std
                        freq_sig_snr[i] = freq_sig_base_val[i][-1]/freq_sig_val_thresh[i]
                    self.baseline_count = 0
                    self.setting_baseline = False
                ...
                '''for i in range(freq_array_len):
                    ...
                    '''
                print("alpha snr = {}".format(freq_sig_snr[alpha_index]))
                self.Yes_size = Y_m*freq_sig_snr[alpha_index]+Y_c
                if self.Yes_size > self.checker_size *2 :
                    self.Yes_size = self.checker_size*2
                elif self.Yes_size < self.checker_size/2:
                    self.Yes_size = self.checker_size/2
                self.No_size = self.checker_size*2-self.Yes_size+self.checker_size/2
